...
int main(int argc, char *argv[])
{
	...
	if (listenSocketFD < 0) { perror("ERROR opening socket"); exit(2); }

	// Enable the socket to begin listening
	if (bind(listenSocketFD, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0){ // Connect socket to port
		perror("OTP_ENC_D: ERROR on binding");
		exit(2);
	}
	...
	while(1){
		...
		switch(spawn_pid){
			case 0: //child
				...
				do{
					memset(buffer, '\0', sizeof(buffer));
					//copy ciphertext characters into a buffer
					strncpy(buffer, &ciphertext[total_chars_sent], PACKET_SIZE);
				...
				}while(total_chars_sent < strlen(ciphertext));
...
//returns containing the encrypted message with a $ at the end
//$ is appended for sending over the network, signals end of message
char* encrypt(char* plaintext, char* key){
	char* ciphertext = malloc(sizeof(char) * (strlen(plaintext) + 2));
	memset(ciphertext, '\0', sizeof(char) * (strlen(plaintext) + 2));
	
	int i, num;
	for(i = 0; i < strlen(plaintext); i++){
		num = ctoi(plaintext[i]) + ctoi(key[i]);
